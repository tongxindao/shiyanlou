...
def cal_social_security(salary):
    ...
    social_security_tax = pension + medical + unemployment \
...
def main():
    try:
        ...
        for arg in sys.argv[1:]:
            ...
            after_salary = after_salary_tax(salary, 
                            social_security, tax)
            
            information[job_number] = after_salary

            print("{0}:{1}".format(job_number, 
                Decimal(information[job_number]).quantize(
                Decimal(".01"))))
    except IndexError:
